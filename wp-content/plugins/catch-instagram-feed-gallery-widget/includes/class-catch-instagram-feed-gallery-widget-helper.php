...
class Catch_Instagram_Feed_Gallery_Widget_Helper {
	...
	function display( $options ) {
		...
		if ( false === ( $data = get_transient( 'catch_insta_json_' . $options['number'] . '_' . $options['size'] ) ) ) {
		  // It wasn't there, so regenerate the data and save the transient
		  $data = $this->get_media( $options['number'] );
		  $data = json_decode( $data, true );
		  set_transient(  'catch_insta_json_' . $options['number'] . '_' . $options['size'] , $data, HOUR_IN_SECONDS );
		}
